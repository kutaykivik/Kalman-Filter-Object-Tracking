...
{
    /* rand() function generates a value between 0 and RAND_MAX (typically 32767).
       This value is scaled to fit the desired range. */
...
static void prvQueueReceiveTask(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        taskENTER_CRITICAL();
        {
            ...
            /* Log estimated data and Kalman gain */
            fprintf(file, "Estimated pos_x: %f, Estimated v_x: %f, Estimated pos_y: %f, Estimated v_y: %f, Kalman gain1: %f, Kalman gain2: %f, ",
                kf.est[0], kf.est[1], kf.est[2], kf.est[3], kf.KGData[0], kf.KGData[2]);
